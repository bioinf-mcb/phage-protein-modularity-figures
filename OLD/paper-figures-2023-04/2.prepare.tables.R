...
anti_defense_descriptions =read.xlsx(ANTI_DEFENSE_DESCRIPTION_PATH, sheetName = "Anti-defense & related", endRow = 246) %>%
  ...
  #mutate(Function = if_else(Function == " Type III R-M system; adenine-specific DNA-methyltransferase", "Type III R-M system; adenine-specific DNA-methyltransferase" , Function)) %>%
  #mutate(Function = if_else(is.na(Function), "unknown antidefense", Function))
...
annotation.indexes = hhr.annotated %>%
  distinct(annotation, category) %>% 
  arrange(category, annotation) %>%
  mutate(annotation.index = row_number())


relaxely.annotated.proteins.including.multi.annot = GetPhrogAnnotationsData(hhr.annotated, annotation.indexes, min.cov = MAIN.COVS.FOR.ANNOTATION[1], min.prob = DEFAULT.MINIMUM.PROB.FOR.ANNOTATION, min.eval = DEFAULT.NAXIMUM.EVAL.FOR.ANNOTATION)
mid.annotated.proteins.including.multi.annot = GetPhrogAnnotationsData(hhr.annotated, annotation.indexes, min.cov = MAIN.COVS.FOR.ANNOTATION[2], min.prob = DEFAULT.MINIMUM.PROB.FOR.ANNOTATION, min.eval = DEFAULT.NAXIMUM.EVAL.FOR.ANNOTATION)
surely.annotated.proteins.including.multi.annot = GetPhrogAnnotationsData(hhr.annotated, annotation.indexes, min.cov = MAIN.COVS.FOR.ANNOTATION[3], min.prob = DEFAULT.MINIMUM.PROB.FOR.ANNOTATION, min.eval = DEFAULT.NAXIMUM.EVAL.FOR.ANNOTATION)
...
#table(unaccepted.genes$annotation)
#known.functions.wrong.chars.names = unaccepted.genes %>%
#left_join(surely.annotated.proteins) %>%
#  mutate(annotation = if_else(is.na(annotation), "unknown", annotation))%>% filter(annotation != "unknown") 
#all.cds[which(names(all.cds) %in% annotated.reprseq.with.unallowed.chars$ncbi.id)]
