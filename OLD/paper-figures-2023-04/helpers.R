...
GetUniquelyAnnotatedProteins = function(annotated.proteins.including.multi.annot) {
  ...
  antidefense.proteins = annotated.proteins.including.multi.annot %>%
    ...
    group_by(qname, num.annots, family, category) %>%
    summarise(annotation = if_else(num.antidefense.annots > 1,  "antidefense", annotation),
              annotation.index = if_else(num.antidefense.annots > 1,  as.integer(0), annotation.index),
              include = if_else(num.antidefense.annots > 1,  TRUE, include)) %>%
    ungroup() %>%
    select(qname, annotation.index, annotation, category, family, include)
...
Get.Pairs.That.Share.T = function(ecods.across.fold.types.data) {
  ecods.across.fold.types.data.stripped = ecods.across.fold.types.data %>%
    select(t_id, x_id, fold.type, all.x, num.x)
  
  fold.type.pairs.that.share.t = ecods.across.fold.types.data.stripped %>%
    inner_join(ecods.across.fold.types.data.stripped, by = c("t_id")) %>%
    filter(fold.type.x != fold.type.y) %>%
    left_join(ecods.across.fold.types %>% distinct(fold.type, x_id, annotation.index) %>% select(fold.type.x = fold.type, x.id.x = x_id, annotation.index.x = annotation.index)) %>%
    left_join(ecods.across.fold.types %>% distinct(fold.type, x_id, annotation.index) %>% select(fold.type.y = fold.type, x.id.y = x_id, annotation.index.y = annotation.index)) 
...
Get.Pairs.That.Share.T.And.Disshare.X = function(ecods.across.fold.types.data) {
  ...
  fold.type.pairs.that.share.t.and.disshare.x = fold.type.pairs.that.share.t %>%
    ...
    group_by(fold.type.x, fold.type.y, t_id) %>%
    mutate(all.x.within.both.fold.types = paste(sort(unique(c(x.id.x, x.id.y))), collapse = " & ")) %>%
    ungroup() %>%
    left_join(ecods.across.fold.types.data %>% distinct(fold.type, all.x, annotation.index) %>% select(all.x.within.fold.type.x =all.x, fold.type.x = fold.type, annotation.index.x = annotation.index), by = "fold.type.x") %>%
    left_join(ecods.across.fold.types.data %>% distinct(fold.type, all.x, annotation.index) %>% select(all.x.within.fold.type.y =all.x, fold.type.y = fold.type, annotation.index.y = annotation.index), by = "fold.type.y") %>%
...
Calculate.Num.Mosaic.Fold.Types = function(ecods.across.fold.types.data) {
  fold.type.pairs.that.share.t.and.disshare.x = Get.Pairs.That.Share.T.And.Disshare.X(ecods.across.fold.types.data) 
   mosaic.pairs.num = fold.type.pairs.that.share.t.and.disshare.x %>%
    ...
    group_by(annotation.x, category.x, annotation.index.x, annotation.y, category.y, annotation.index.y)  %>%
...
Calculate.Mosaicism.Per.Fold.Type = function(ecods.across.fold.types.data) {
  ...
  all.fold.type.pairs.sharing.t = fold.type.pairs.that.share.t 
    distinct(fold.type.x, fold.type.y, annotation, category, annotation.index) %>%
    rowwise() %>%
    mutate(pair = paste(sort(c(fold.type.x, fold.type.y)), collapse = " AND ", sep = "")) %>% 
    group_by(annotation, category, annotation.index)  %>%
 ...
 mosaic.pairs.num = fold.type.pairs.that.share.t.and.disshare.x %>%
  ...
  mosaicism.per.fold.type   = ecods.across.fold.types.data %>% 
    distinct(annotation, category, annotation.index) %>%
...
Plot.Domains.Within.Mosaic.Proteins = function(domains.within.proteins.domain.mosaic) {
  domains.within.proteins.with.significant.higher.domain.mosaic = domains.within.proteins.domain.mosaic %>%
    ...
    select(t_id, t_name, mosaic,odds.ratio, corrected.pval) %>%
  ...
  included.ecods = domains.within.proteins.domain.mosaic %>% 
  ...
  ecod.table <- domains.within.proteins.domain.mosaic %>% 
