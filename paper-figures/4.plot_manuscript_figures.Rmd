...
plot.data = rbind(annotation.uncertainty.data.vs.cov %>% mutate(Figure = "Fig_S2_A"),
                  annotation.uncertainty.data.vs.prob %>% mutate(Figure = "Fig_S2_B"))  %>%        
  mutate(annotation.coverage = annotation.coverage*100) %>%
  rename(probability_threshold = minimum.probability,
         sequence_coverage_threshold = annotation.coverage)

p.a=ggplot(data = plot.data %>%
             filter(Figure == "Fig_S2_A") %>%
             mutate(`probability\nthreshold [%]` = as.factor(probability_threshold))) +
  geom_point(aes(x = sequence_coverage_threshold, y = perc.unique.annotation, linetype = `probability\nthreshold [%]`), col = col1.uniqueness, size = point.size) + 
  geom_line(aes(x = sequence_coverage_threshold, y = perc.unique.annotation, linetype = `probability\nthreshold [%]`), col = col1.uniqueness, size = line.size) + 
  geom_point(aes(x = sequence_coverage_threshold, y = perc.annotated, linetype = `probability\nthreshold [%]`), col = col2.coverage, size = point.size) + 
  geom_line(aes(x = sequence_coverage_threshold, y = perc.annotated, linetype = `probability\nthreshold [%]`), col = col2.coverage, size = line.size) + 
...
p.b=ggplot(data = plot.data %>%
          filter(Figure == "Fig_S2_B") %>%
          mutate(`sequence coverage\nthreshold [%]`  = as.factor(sequence_coverage_threshold))) +
  geom_point(aes(x = probability_threshold, y = perc.unique.annotation, linetype = `sequence coverage\nthreshold [%]` ), col = col1.uniqueness, size = point.size) + 
  geom_line(aes(x = probability_threshold, y = perc.unique.annotation, linetype = `sequence coverage\nthreshold [%]` ), col = col1.uniqueness, size = line.size) + 
  geom_point(aes(x = probability_threshold, y = perc.annotated, linetype = `sequence coverage\nthreshold [%]` ), col = col2.coverage, size = point.size) + 
  geom_line(aes(x = probability_threshold, y = perc.annotated, linetype = `sequence coverage\nthreshold [%]` ), col = col2.coverage, size = line.size) + 
  Get.Theme(legend.position = "right") +
  xlab("probability [%]") +
  theme( axis.title.y.right = element_text(colour = col2.coverage, size = text.size),
         axis.text.y.right = element_text(colour = col2.coverage, size = text.size),
...
ggsave(p.figure, filename = sprintf("%sFigure_Supplementary/tradeoff/FigureS2.png", OUTPUT.FIGURES.PATH), bg = "white", width = 12, height = 10)
fwrite(plot.data, file = sprintf("%sSource_Data/FigureS2.txt", OUTPUT.FIGURES.PATH), sep = "\t")
```
...
fwrite(domain.network.selected.categories %>% 
         filter(category == "DNA, RNA and nucleotide metabolism"), 
       file = sprintf("%sSource_Data/FigureS7.txt", OUTPUT.FIGURES.PATH), sep = "\t")

fwrite(domain.network.selected.categories %>% 
         filter(category == "tail"), 
       file = sprintf("%sSource_Data/FigureS8.txt", OUTPUT.FIGURES.PATH), sep = "\t")

fwrite(domain.network.selected.categories %>% 
         filter(category == "lysis"), 
       file = sprintf("%sSource_Data/FigureS9.txt", OUTPUT.FIGURES.PATH), sep = "\t")
...
fwrite(rbind(
  annotation.netwok.default.threshold %>% mutate(Figure = "Fig_S3_A"), 
  annotation.netwok.relaxed.threshold %>% mutate(Figure = "Fig_S3_B")),
       file = sprintf("%sSource_Data/FigureS3.txt", OUTPUT.FIGURES.PATH), sep = "\t")
```
...
fwrite(mosaic.network,
       file = sprintf("%sSource_Data/Figure2B_network.txt", OUTPUT.FIGURES.PATH), sep = "\t")
fwrite(included.categories.metadata,
       file = sprintf("%sSource_Data/Figure2B_nodes_attributes.txt", OUTPUT.FIGURES.PATH), sep = "\t")
...
fwrite(h.domain.occurence.data,
       file = sprintf("%sSource_Data/Figure1.txt", OUTPUT.FIGURES.PATH), sep = "\t")
...
fwrite(num.f.within.h,
       file = sprintf("%sSource_Data/FigureS5.txt", OUTPUT.FIGURES.PATH), sep = "\t")
...
fwrite(t.domain.occurence.data.full,
       file = sprintf("%sSource_Data/FigureS4.txt", OUTPUT.FIGURES.PATH), sep = "\t")
...
dev.off()
fwrite(all.tiles.data,
       file = sprintf("%sSource_Data/Figure3.txt", OUTPUT.FIGURES.PATH), sep = "\t")
...
cor.test(diversity.ok.ecod$num.families, diversity.ok.ecod$num.topology.combinations,
...
fwrite(diversity.ok.ecod,
       file = sprintf("%sSource_Data/FigureS11.txt", OUTPUT.FIGURES.PATH), sep = "\t")
...
```{r}
# Data THAT REQUIRES DOTS
num.h.per.q = data.table::fread(sprintf("%sFigure_Supplementary/ecod-coverage/num.h.per.q.txt", OUTPUT.FIGURES.PATH)) %>%
  select(annotation.index, value = num.h) %>% mutate(proportion = "max.num.h")
prot.length.per.qname = data.table::fread(
  sprintf("%sFigure_Supplementary/ecod-coverage/prot.length.per.qname.txt", OUTPUT.FIGURES.PATH)) %>%
  select(annotation.index, value = length) %>% mutate(proportion = "mean.length")
ecods.across.fold.types = data.table::fread(
  sprintf("%sFigure_Supplementary/ecod-coverage/ecods.across.fold.types.txt", OUTPUT.FIGURES.PATH)) %>%
  select(annotation.index, value = num.t) %>% mutate(proportion = "mean.num.t")

point.data = num.h.per.q %>% rbind(prot.length.per.qname) %>% rbind(ecods.across.fold.types) %>%
  left_join(annotation.indices) %>%
    inner_join(diversity.top %>% distinct(annotation, category)) %>%
  distinct(annotation, category, value, proportion) 
  ...
  levels = c("mean.length",
            ...
            "mean.num.t", 
             ...
             "prop.with.multi.x.domains")
  diversity.top$proportion = factor(diversity.top$proportion, levels = levels)
  point.data$proportion = factor(point.data$proportion, levels = levels)

    p <- ggplot(mapping = aes(y = annotation, x = value, fill = category)) +
    geom_bar(data = diversity.top, stat = "identity", position=position_dodge()) +
    geom_point(data = point.data, size = 0.1) +
...
ggsave(sprintf("%sFigure_Supplementary/ecod-coverage/ecod-coverage2.pdf", OUTPUT.FIGURES.PATH), 
      p, width = 15, height = 11)
ggsave(sprintf("%sFigure_Supplementary/ecod-coverage/ecod-coverage2.png", OUTPUT.FIGURES.PATH), 
     p, bg = "white", width = 15, height = 11)

  
  p <- ggplot(mapping = aes(y = annotation, x = value, fill = category)) +
    geom_bar(data = diversity.top, stat = "identity", position=position_dodge()) +
    geom_jitter(data = point.data, size = 0.02, width = 0, alpha = 0.5) +
    scale_fill_manual(values = PHROG.COLOR.MAP) + theme_minimal() + 
    theme(strip.text.y = element_text(face = "bold", size = 7, angle = 360)) + 
    labs(x = "", y = "") +
    guides(fill = "none")  +
...
ggsave(sprintf("%sFigure_Supplementary/ecod-coverage/ecod-coverage3.pdf", OUTPUT.FIGURES.PATH), 
      p, width = 15, height = 11)
ggsave(sprintf("%sFigure_Supplementary/ecod-coverage/ecod-coverage3.png", OUTPUT.FIGURES.PATH), 
     p, bg = "white", width = 15, height = 11)



fwrite(point.data,
       file = sprintf("%sSource_Data/FigureS10.txt", OUTPUT.FIGURES.PATH), sep = "\t")
...
mosaicism.underestimated.data.raw  = data.table::fread(sprintf("%sFigure4/mosaic.domain.classes.txt", OUTPUT.FIGURES.PATH)) %>%
...
mosaicism.underestimated.data = mosaicism.underestimated.data.raw %>%
...
fwrite(mosaicism.underestimated.data.raw,
       file = sprintf("%sSource_Data/Figure4.txt", OUTPUT.FIGURES.PATH), sep = "\t")
```
...
p.odds = Plot.Domains.Within.Mosaic.Proteins(all.t.in.fold.types, colors = domain_fill_colors, significance_level_alpha = significance_level_alpha) + theme(text = element_text(size = 20))
...
p.odds = Plot.Domains.Within.Mosaic.Proteins(all.t.in.fold.types_pident50, colors = domain_fill_colors, significance_level_alpha = significance_level_alpha) + theme(text = element_text(size = 20))
...
fwrite(rbind(
  all.t.in.fold.types %>% mutate(Figure = "Figure_2_C"),
  all.t.in.fold.types_pident50 %>% mutate(Figure = "Figure_2_D")),
       file = sprintf("%sSource_Data/Figure2_CD.txt", OUTPUT.FIGURES.PATH), sep = "\t")
```
...
cov.vs.pident.plot = ggplot() + 
  stat_binhex(data = cov.vs.pident, #%>% select(qname,sname,pident,max.cov), 
              bins = 50, aes(x=pident, y=max.cov)) + 
...
fwrite(rbind(
  cov.vs.pident %>% select(qname,sname,pident,max.cov) %>% mutate(Figure = "Fig_S12_A"),
  not.domain.mosaic.pair.cov.vs.pident.data %>% select(qname,sname,pident,max.cov) %>% mutate(Figure = "Fig_S12_B"),
  not.domain.mosaic.pair.cov.vs.pident.data  %>% select(qname,sname,pident,max.cov) %>% mutate(Figure = "Fig_S12_C")),
       file = sprintf("%sSource_Data/FigureS12.txt", OUTPUT.FIGURES.PATH), sep = "\t")
```
...
fwrite(rbind(
 recent_HGT_pairs_70 %>% mutate(Figure = "Fig_S13_A"),
 recent_HGT_pairs_90 %>% mutate(Figure = "Fig_S13_B")),
       file = sprintf("%sSource_Data/FigureS13.txt", OUTPUT.FIGURES.PATH), sep = "\t")
```
...
all.t.in.fold.types = data.table::fread(file = sprintf("%sFigure2/all.t.in.fold.types.txt", OUTPUT.FIGURES.PATH))
stat.dat = all.t.in.fold.types  %>% select(t_id, t_name, h.index, h_name, odds.ratio, pval, corrected.pval) %>%
  full_join(all.t.in.fold.types.pident50 %>% select(t_id, t_name, h.index, h_name, pident50.odds.ratio = odds.ratio, pident50.pval = pval, pident50.corrected.pval = corrected.pval))
