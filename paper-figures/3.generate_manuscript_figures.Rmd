...
```{r}
ecod.domains.with.annotations = ecod.domains.hits %>% 
  filter(minimum.prob == MINIMUM.PROB.FOR.DOMAIN) %>%
  distinct(qname, x_id, h_id, t_id, f_id) %>%
...
for (selected_category in selected_categories) {
  ...
  domain.network.selected.categories.this.cat = nucl.metabolism.domain.hits %>%
    ...
    #filter(num.prot >= 5)
    domain.network.selected.categories = rbind(domain.network.selected.categories, domain.network.selected.categories.this.cat)
}
...
data.table::fwrite(domain.network.selected.categories, sprintf("%sFigure5/domain.network.selected.categories.txt", OUTPUT.FIGURES.PATH))
...
recent_HGT_pairs = rbind( recent_HGT_pairs_raw %>%
  ...
  filter(from <= to) %>%
  distinct() 
data.table::fwrite(recent_HGT_pairs, sprintf("%sFigure_Supplementary/FigS11_recent_HGT/recent_HGT_network.txt", OUTPUT.FIGURES.PATH))
```
...
mosaicism.per.fold.type = Calculate.Mosaicism.Per.Fold.Type(ecods.across.fold.types.data, all.x.and.t.within.fold.type)
...
mosaicism.per.family = Get.Num.Mosaic.Family.Pairs(all.family.pairs.sharing.t.and.disshare.x, all.family.pairs.sharing.t)
...
data.table::fwrite(diversity.data2, sprintf("%sFigure_Supplementary/FigS9_ecod_hist/Diversity_vs_mosaicism.txt", OUTPUT.FIGURES.PATH))
...
seq.mosaicism.from.various.classes.within.category = protein.similarity.data %>%
  inner_join( surely.annotated.proteins %>% distinct(qname, category) %>% select(qname = qname, qannot = category)) %>%
  inner_join( surely.annotated.proteins %>% distinct(qname, category) %>% select(sname = qname, sannot = category)) %>%
  filter(qannot == sannot) %>%
  group_by(qannot) %>%
  summarise(prop.mosaic.pairs.sequence.thr0p1 = sum(mosaic.pident10)/n(),
            prop.mosaic.pairs.sequence.thr0p3 = sum(mosaic.pident30)/n(),
...
diversity.data.per.category = surely.annotated.proteins %>%
  distinct(category) %>%
...
data.table::fwrite(diversity.data.per.category, sprintf("%sFigure4/mosaicism.per.category.txt", OUTPUT.FIGURES.PATH))
```
