...
ecod.domain.descriptions$h_id <- sapply(1:nrow(ecod.domain.descriptions), function(index){
  ...
  this.topology.h.index <- as.numeric(paste(this.topology.h.index, collapse = "."))
})


num.f.within.h = 
ecod.domain.descriptions %>% group_by(h_id) %>% summarise(num_f = n_distinct(f_id), num.t = n_distinct(t_id)) %>%
  inner_join(h.domain.occurence.data %>% distinct(h_id, h_name, all.categories)) %>%
  distinct(h_name, num_f, all.categories)
...
```{r}
nucl.metabolism.domains = surely.annotated.ecod.domains.high.confidence %>%
...
nucl.metabolism.domain.hits = surely.annotated.ecod.domains.high.confidence %>%
  inner_join(nucl.metabolism.domains) %>%
  distinct(qname, t_id)
nucl.metabolism.domain.network = nucl.metabolism.domain.hits %>%
  inner_join(nucl.metabolism.domain.hits, by = c("qname")) %>%
  filter(t_id.x < t_id.y) %>%
  rowwise() %>%
  mutate(pair = paste(sort(c(t_id.x, t_id.y)), collapse = " AND ", sep = "")) %>%
  group_by(t_id.x, t_id.y, pair) %>%
  summarise(num.prot = n_distinct(qname)) %>%
  ungroup() %>%
  select(from = t_id.x, to = t_id.y, num.prot) %>%
  filter(num.prot >= 5)
...
domain.annotations = surely.annotated.ecod.domains.high.confidence %>%
  filter(category == "DNA, RNA and nucleotide metabolism")  %>%
  distinct(t_id, annotation) %>%
  group_by(t_id) %>%
  mutate(n = n_distinct(annotation)) %>%
  arrange(t_id) %>%
  summarise(annotation = if_else(all(n == 1), annotation[1], "multiple")) %>%
  mutate(multi = annotation == "multiple")  %>%
  mutate(color = if_else(multi, "black", "blue"))

annotated.domain.network = Annotate_Graph(nucl.metabolism.domain.network, 
                  edge.colors = NULL,
                  node.color.map = domain.annotations %>%
                                      select(node = t_id, color),
...
ggsave(annotated.domain.network.figure, filename = sprintf("%sFigure5/annotated.domain.network.figure.png", OUTPUT.FIGURES.PATH), width = 8, height = 8)
