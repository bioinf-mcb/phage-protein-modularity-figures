...
av.num.h.per.qname = surely.annotated.ecod.domains.high.confidence %>% 
...
prot.lengths = surely.annotated.proteins.from.included.categories %>%
...
# ceck how heterogeneous families are
```{r}
# !! many Families contain proteins of multiple annotations and also proteins that are mosiac and proteins that are non-mosaic
# each family needs to be either mosaic or not to do the test !! 
# if we use n_distinct(family[mosaic]) and n_distinct(family[non.mosaic]) we will count some families twice

unique.na.rm = function(x) {
  y = x[!is.na(x)] 
  return(unique(y))
  }
all.families.tmp = all.proteins %>% 
  left_join(surely.annotated.proteins.from.included.categories)  %>%
  inner_join(included.category.sizes) %>%
  group_by(family) %>%
  summarise(mosaic.homogeneous = length(unique(mosaic)) == 1,
            domain.mosaic.homogeneous = length(unique(mosaic.domain)) == 1,
            mosaic = any(mosaic),
            mosaic.domain = any(mosaic.domain),
            annotation.homogeneous = length(unique.na.rm(annotation.index))==1)


# families cannot be uniquely assigned to the groups: mosaic, non-mosaic, this.annotation, other annotation
# se we cannot perfrom ch-square test on them
# We can perform chi-square test on # proteins though 


# How do we trrat a case when family contains mosaic tail fibers and non-mosaic spikes?
# at the moment we count is as non-mosaic for spikes and as mosaic for fibers.


all.families.tmp2 = all.proteins %>% 
    left_join(surely.annotated.proteins.from.included.categories)  %>%
  inner_join(included.category.sizes) %>%
  group_by(family, annotation.index) %>%
  summarise(mosaic = any(mosaic),
            mosaic.domain = any(mosaic.domain))

# are there any cases like that? yes! eg fam00004 is mosaic for tail completion connector and non-mosaic for minor capsid
all.families.tmp2 %>% 
  group_by(family) %>%
  summarise(m = length(unique(mosaic)) == 1,
            m2 = length(unique(mosaic.domain)) ==1) %>%
  filter(!m | !m2)
...
mosaic.domain.classes.old = all.proteins %>%
  left_join(surely.annotated.proteins.from.included.categories) %>%
  inner_join(included.category.sizes) %>%
  anti_join(unspecific.annotation.indices) %>%
  group_by(annotation.index, num.proteins, num.families) %>%
  summarise(num.prot.mosaic.domain = sum(mosaic.domain),
